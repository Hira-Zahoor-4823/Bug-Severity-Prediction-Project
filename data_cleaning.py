...
# Dataset: Frontend UI/UX Bug Dataset (50,000 records)
...
# Step 1: We are loading the dataset here

print("=" * 60)
print("Step no 1: Loading Dataset")
print("=" * 60)

df = pd.read_csv("frontend_uiux_bug_dataset_50000.csv")

print(f"Dataset has been loaded successfully!")
print(f"Shape: {df.shape[0]} rows x {df.shape[1]} columns")
print(f"\nColumn names: {list(df.columns)}")

# Step 2: We are just initially exploring the dataset for data types and columns etc
...
# Step 3: We are looking for missing values in data set here:
...
if len(has_missing) == 0:
    ...
    # Here we are filling the missing numerical values with median. we did not use mean because it gets affected by the outlier values.
    ...
    # We are filling the missing categorical/text values with mode(the most repeated value) because mean and median cannot be computed for these
...
# Step 4: We are checking the rows if duplicate ones exist and removing those 
...
# Step 5: We are standardizing the column names to remove any white spaces or capital letters here
...
# Step 6: Here we are cleaning the categorical columns data
...
# fixing the inconsistent bug_type of "Animation Bug" to "Animation" because the others just show words of type of bugs
...
# Step 7: Here we are just validating the target columns of severity and priority to make sure that they only contain certain categorical values
...
# Step 8: Here we are cleaning the numerical columns
...
for col in numerical_cols:
    if col in df.columns:
        ...
        # Removing negative values because logically negative values cannot exist for time or score
        neg_count = (df[col] < 0).sum()
        if neg_count > 0:
            print(f"  '{col}': {neg_count} negative values → set to NaN")
            df.loc[df[col] < 0, col] = np.nan

        # Filling NaN with the median value
...
# Step 9: Now cleaning the boolean cloumn values:

print("\n" + "=" * 60)
print("Step 9: Cleaning the boolean columns:")
print("=" * 60)

# Instead of that we use Yes and No strings in boolean columns, we converted to proper True and False values
...
# Step 10: Here we are cleaning the tag columns: the semi colon separated strings
...
# Step 11: Here we are dropping the irrelevant columns after analysis of the dataset

print("\n" + "=" * 60)
print("Step 11: Dropping irrelevant columns:")
print("=" * 60)

#page url has the same data as the module column so there is no need to keep it
...
# Step 12: Here we are resetting the index values to avoid the wrong numbering(due to the cleaning)
...
# Step 13: Here we are showing the final summary report after cleaning
...
# STEP 14: Here we are additionally generating some visualizations to visually see the results for ease and saving it in a separate folder:
...
# Now generating Severity vs Priority Heatmap
plt.figure(figsize=(8, 5))
...
# User Impact Score Distribution
plt.figure(figsize=(9, 5))
...
# for representing resolved vs the unresolved bugs
...
# representing the top 10 modules by bug count values
