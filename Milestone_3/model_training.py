...
plt.tight_layout()
plt.savefig(f"{OUTPUT_DIR}/01_accuracy_comparison.png", dpi=150); plt.close()
...
for ax, df_mi, col, title, color in [
    ...
    (axes[1], df_mi_p.sort_values("MI_Priority"),     "MI_Priority", "Priority", "mediumorchid"),
]:
...
plt.tight_layout()
plt.savefig(f"{OUTPUT_DIR}/07_model_summary_table.png", dpi=150); plt.close()
...
json_path = f"{OUTPUT_DIR}/model_summary.json"
...
joblib.dump(encoders, os.path.join(SCRIPT_DIR, "data_encoders.joblib"))

print(f"  Saved: severity_rf_model.joblib  (model: {best_s})")
print(f"  Saved: priority_rf_model.joblib  (model: {best_p})")
print(f"  Saved: data_encoders.joblib")
